...
class Config(BaseSettings):
    ...
    use_mempool_api: bool = Field(
        default=True, description="Use Mempool.space API instead of Bitcoin RPC"
    )
    mempool_api_url: str = Field(
        default="https://mempool.space/api", description="Mempool.space API URL"
    )
    ...
    coingecko_api_url: str = Field(
        default="https://api.coingecko.com/api/v3", description="CoinGecko API URL"
    )
    ...
    check_interval: int = Field(
        default=10, description="Interval in seconds between checks"
    )
    ...
    enable_twitter: bool = Field(
        default=True, description="Enable Twitter notifications"
    )
