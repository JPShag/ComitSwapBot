...
class Transaction(BaseModel):
    ...
    locktime: int = Field(default=0, description="Earliest block height for confirmation")
    ...
    block_time: Optional[datetime] = Field(None, description="Block timestamp if confirmed")
...
class HTLCTransaction(Transaction):
    ...
    script_details: Optional[HTLCScript] = Field(None, description="Parsed HTLC script components")
    value_sats: int = Field(description="Output value in satoshis")
    output_index: int = Field(description="Index of the HTLC output")
    revealed_secret: Optional[str] = Field(None, description="32-byte secret for redeem transactions")
...
class AtomicSwap(BaseModel):
    ...
    lock_transaction: HTLCTransaction = Field(description="Initial HTLC setup transaction")
    redeem_transaction: Optional[HTLCTransaction] = Field(None, description="Secret reveal transaction")
    refund_transaction: Optional[HTLCTransaction] = Field(None, description="Timeout refund transaction")
    ...
    xmr_amount: Optional[Decimal] = Field(None, description="Equivalent XMR amount at detection time")
    btc_xmr_rate: Optional[Decimal] = Field(None, description="BTC/XMR exchange rate used")
    ...
    notification_sent: Optional[str] = Field(None, description="Twitter tweet ID if posted")
